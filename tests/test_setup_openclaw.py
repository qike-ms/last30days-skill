...
class TestRunGithubAuth:
    ...
    def test_gh_token_not_used(self, mock_which, mock_subproc, mock_device):
        """Local gh tokens are never read or sent to ScrapeCreators."""
    ...
    @patch("lib.setup_wizard.run_full_device_auth")
    def test_goes_to_device(self, mock_device):
        """GitHub auth always uses device flow."""
        mock_device.return_value = {
            "status": "success", "method": "device",
            "api_key": "sc_live_fromDevice",
        }
